...
Runner.prototype.runTestModule = function(modulePath, fullPaths) {
  ...
  return this.currentTestSuite
    .on('testcase:finished', function(results, errors, time) {
      var isTestcaseUnstable = this.currentTestSuite.module.isUnstable();
      var tests = this.globalResults.modules[moduleKey].completed[this.currentTestSuite.currentTest] = {
        passed  : isTestcaseUnstable ? 0 : results.passed,
        failed  : isTestcaseUnstable ? 0 : results.failed,
      ...
      // If testcase is marked unstable, do not increment no. failed and errors
      if (!isTestcaseUnstable) {
