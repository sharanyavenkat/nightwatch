...
class TestSuite extends EventEmitter {
  ...
  runGlobalHook(hookName) {
    ...
    return this.handleRunnable(hookName, () => {
      return this.globalHooks[hookName].run(this.api);
    });
  }
  ...
  runTestSuite() {
    return this.createSession()
      ...
      .then(failures => {
        return this.terminate(failures && !this.context.isTestUnstable() ? 'FAILED': '')
          .catch(err => {
  ...
  terminate(reason = 'SIGINT') {
    ...
    let runnable = new Runnable('terminate', () => {
      this.api.end();
    });
  ...
  runCurrentTest(testName) {
    ...
    this.setReporterCurrentTest().resetQueue();
